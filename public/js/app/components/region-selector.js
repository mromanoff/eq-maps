(function (global, App) {
    ...
    RegionSelector.prototype = {
        ...
        JST: {
            select: _.template(
                '<img src="<%- selectedRegion.BackgroundImage %>" alt="<%- selectedRegion.Name %>">\
                <span class="select-wrapper select-regions" data-component="inline-select">\
                    <select name="regions">\
                        <option value="Select-Region">Select Region</option>\
                        <%= this.option({regionsData: regionsData}) %>\
                    </select>\
            ...
            option: _.template (
                '<% _.each(regionsData, function (region) { %>\
                    <option value="<%- region.ShortName.toLowerCase() %>"><%- region.Name %></option>\
                <% }, this); %>'
            )
        }
    };
