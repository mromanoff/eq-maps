(function(global) {
    var DEVICE_MAXIMUM_WIDTH_SMALL = 768, DEVICE_MAXIMUM_WIDTH_MEDIUM = 1024, CB_QUEUE = [], LOADED_FAVS = false, EQ = global.EQ || {}, Helpers = {};
    var data_cache = [];
    ...
    Helpers.getDeviceSize = function() {
        var w = (document.documentElement || document.body).clientWidth, deviceSize;
    ...
    Helpers.getFacilityById = function(facilityId) {
        var facilitiesArray = this.getAllFacilities(), facilityFound = null;
        $.each(facilitiesArray, function(i, facility) {
    ...
    Helpers.getUserDefaultRegion = function(clubId) {
        ...
        region = _.find(facilities, {
            Id: userProfileJson.FacilityId
        });
        if (region !== undefined) {
    ...
    Helpers.getClosestWithin = function(availableLocations, currentLocation) {
        var candidate = null, maxDistance;
    ...
    Helpers.getDevicePlatform = function() {
        var agent = navigator.userAgent, candidate;
    ...
    Helpers.getDeviceMapURL = function(club) {
        ...
        if (href[Helpers.getDevicePlatform()]) {
            href = href[Helpers.getDevicePlatform()].replace("{googleAddress}", club.GoogleAddress);
        } else {
    ...
    Helpers.dateTime = {
        getCurrentWeek: function(date) {
            var today = moment(date), monday, sunday;
            monday = moment(today).startOf("isoWeek"), sunday = moment(monday).add("days", 7);
            return {
        ...
        getCurrentDay: function(date) {
            var today = moment(date), tomorrow = moment(today).add("days", 1);
            return {
        ...
        getTimeRange: function(startTime, endTime, twentyfour) {
            var start = moment(startTime), end = moment(endTime), format = "";
            if (twentyfour) {
                if (endTime) {
        ...
        convertDateToString: function(rawDate, fullMonth) {
            ...
            monthString = fullMonth ? "MMMM" : "MMM", dateString = EQ.Helpers.user.getUserCountry() === "US" ? "dddd, " + monthString + " D" : "dddd, D " + monthString;
            return date.format(dateString);
        },
        formatDisplayTime: function(displayTimeString) {
            var displayTime = displayTimeString.substr(0, 19), regionInfo = displayTimeString.substr(19), AMcount, PMcount;
    ...
    Helpers.user = {
        ...
        getFavorites: function(callback, errcallback) {
            var endpoint = APIEndpoint + "/me/favorites", that = this;
                ...
                if (!LOADED_FAVS) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            var favoritesData = data, finalFavoritesData = {
                                categories: "",
                                instructors: "",
                                classes: "",
                                clubs: "",
                            ...
                            _.each(finalFavoritesData, function(arr, cat, obj) {
                                obj[cat] = _.sortBy(arr, function(o) {
    ...
    Helpers.loaderAndErrorHandler = function($el, options) {
        var LoaderError, loaderDiv, errorDiv, loaderOverlay, errorOverlayDiv;
        loaderDiv = '<div class="loader hidden">' + '<div class="loader-circles bounce1"></div>' + '<div class="loader-circles bounce2"></div>' + '<div class="loader-circles bounce3"></div>' + "</div>";
        loaderOverlay = '<div class="loader-overlay hidden">' + '<div class="loader-overlay-container">' + '<div class="loader-circles bounce1"></div>' + '<div class="loader-circles bounce2"></div>' + '<div class="loader-circles bounce3"></div>' + "</div>" + "</div>";
        errorDiv = '<div class="error-box error-message-component hidden">' + '<p class="error-title"></p>' + '<p class="error-description"></p>' + "</div>";
        errorOverlayDiv = '<div class="error-box error-overlay hidden">' + '<div class="error-overlay-container">' + '<p class="error-title"></p>' + '<p class="error-description"></p>' + "</div>" + "</div>";
        LoaderError = function() {
            var $errorBox, $loaderBox;
            this.$el = $el;
    ...
    Helpers.PopupMessageHandler = function($el, options) {
        var popupDiv;
        popupDiv = '<div class="overlay-box">' + "<h2></h2>" + "<p></p>" + '<a href="/profile" class="button box small"></a>' + '<a href="#" class="icon-close"></a></div>';
    ...
    Helpers.getQueryStringVariable = function(name) {
        var query = History.getState().url.split("?")[1], vars = query ? query.split("&") : [], result;
        if (vars.length !== 0) {
    ...
    Helpers.numbers = {
        ...
        trimWithFlooring: function(value, decimals) {
            var power = Math.pow(10, decimals || 1);
    ...
    Helpers.getService = function(service, params, opts, multiple) {
        var url = APIBaseUrl + service + Helpers.buildGetParams(params), deferred = new $.Deferred(), isCrossDomain = EQ.Helpers.getBrowser() === "MSIE" && parseInt(EQ.Helpers.getBrowserVersion()) < 10 ? false : true, ajaxOpts = {
        ...
        if (data_cache[url]) {
            ...
            deferred = $.ajax(url, ajaxOpts).fail(Helpers.errorHandler).done(function(response, status, dfObj) {
    ...
    Helpers.deleteService = function(service, params, opts) {
        var url = APIBaseUrl + service, isCrossDomain = EQ.Helpers.getBrowser() === "MSIE" && parseInt(EQ.Helpers.getBrowserVersion()) < 10 ? false : true, ajaxOpts = {
            crossDomain: isCrossDomain,
            contentType: "application/json",
            dataType: "json",
            data: JSON.stringify(params),
    ...
    Helpers.when = function() {
        return $.when.apply(this, arguments).then(function() {
            var args = [];
    ...
    Helpers.convertDistance = function(distanceValue, distanceUnit, system) {
        ...
        var equivalenceKMinMI = .621371192;
...
(function(global) {
    ...
    var MapIcon = function(url, width, height) {
        return new google.maps.MarkerImage(url, new google.maps.Size(width, height), new google.maps.Point(0, 0), new google.maps.Point(width / 2, height / 2));
    };
    ...
    MapMarkers.prototype = {
        add: function(obj, isRich) {
            ...
            var map = this.map, latLng = new google.maps.LatLng(obj.lat, obj.lng), marker;
            if (isRich) {
        ...
        find: function(lat, lng) {
            var candidate = null, lookUpPosition;
    ...
    Map.prototype = {
        ...
        fit: function(markersArr, diffZoom) {
            var bounds = new google.maps.LatLngBounds(), that = this;
...
(function(global) {
    ...
    Validator.prototype = {
        init: function() {
            var inputDataAttributes = this.$el.get(0).attributes, that = this, regexDataAttribute = /^data\-val\-(.+)$/;
        ...
        validate: function() {
            var isFieldValid = true, fieldValidationResult = {
                isValid: true,
                message: ""
            }, validationResult = {}, that = this;
    ...
    var Validations = {
        "data-val-required": function($el) {
            var result = {
                isValid: true,
                message: ""
            }, value = $.trim($el.val()), message = $el.data("valRequired");
        ...
        "data-val-length": function($el) {
            var result = {
                isValid: true,
                message: ""
            }, value = $el.val(), valLengthMax = $el.data("valLengthMax"), valLengthMin = $el.data("valLengthMin"), minCheck = true, maxCheck = true, message = $el.data("valLength");
        ...
        "data-val-regex": function($el) {
            var result = {
                isValid: true,
                message: ""
            }, value = $el.val(), message = $el.data("valRegex"), regexPattern = $el.data("valRegexPattern"), regex = new RegExp(regexPattern.trim(), "i");
            if (!regex.test(value)) {
                result = {
                    isValid: false,
                    message: message
        ...
        "data-val-creditcard": function($el) {
            var result = {
                isValid: true,
                message: ""
            }, value = $el.val().replace(/\s/g, ""), message = $el.data("dataValCreditcard"), checkCreditCard = function luhnChk(luhn) {
        ...
        "data-val-notdefault": function($el) {
            var result = {
                isValid: true,
                message: ""
            }, value = $el.val(), message = $el.data("valNotdefault");
            if (value === "") {
                result = {
                    isValid: false,
                    message: message
...
/*! local_env equinox_maps v1.0.0 - 2015-03-01 08:03:49 */
