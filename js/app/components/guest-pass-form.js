(function (global, App) {
	'use strict';
	/* global Crittercism, EQ, moment, APIEndpoint */

	/**
     * Guest Pass Form module .
     */
	App.Components['guest-pass-form'] = function ($el) {
		...
		GuestPass.BindDropdowns = function () {
			//Create the $regions options and $clubs variable.
			var $regions = '',
                $clubs = {},
                $visitDate = '';
            ...
            //Populate the dateVisit select with the createDate(). // 2 weeks from now.
		...
		GuestPass.Validate = function () {
			var $firstName = $el.find('.first-name'),
                $lastName = $el.find('.last-name'),
                $club = $el.find('.club'),
                $email = $el.find('.email'),
                $phone = $el.find('.phone'),
                $date = $el.find('.date');
			...
			// Validate that there is a selected club
				...
				if ($el.find('.error').length === 0) {
					...
					$.ajax({
						...
						if (!response) {
							...
							//$el.find('.guest-pass-form').addClass('hidden');
							//$el.find('.guest-pass-thanks small').text('Thank you, ' + data.nameFirst);
							//$el.find('.guest-pass-thanks').removeClass('hidden');

							//Tracking [GuestPass Lead Submit]
							// data: formRegion formFacilityId, formFacilityName, formName, formStatus
                            ...
                            // redirect to thank you page with guest first name and email.
		...
		GuestPass.init = function () {
			...
			// On Document Ready, bind the defaults.
			$(function () {
