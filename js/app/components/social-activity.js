    ...
    */
    ...
    var FriendsOnEQView = Backbone.View.extend({
        ...
        updateFriendsCount: function () {
            var $friendsNumber = this.$el.find('span.friendsNumber'),
                $friendsTag = this.$el.find('span.friendsTag'),
                $friendsIsAre = this.$el.find('span.friendsIsAre');
        ...
        render: function () {
            var self = this,
                $carousel = this.$el.find('.fb-friends-list .friends-carousel');
    ...
    var FriendsWithClassesSimpleView = Backbone.View.extend({
        ...
        addClass: function (e) {
            ...
            var classInfo = this.model.get('classInfo'),
                ENDPOINT,
                that = this,
                $link = that.$el.find('a.add-class'),
                $message = that.$el.find('.message');
    ...
    */
    ...
    FacebookClasses.getSocialActivitiesByUserInfo = function (dataUserInfo, $connectSection, $connectButton) {
        ...
        .done(function (data) {
            ...
            // Omniture = DPLAT-1886 Implimentation
            window.tagData.socialStat = window.tagData.socialStat || {};
            window.tagData.socialStat = {
                ...
                eqxFriendsWithClasses: data ? ((data.fbFriendsOnEQ.classesWithFriends !== '' && data.fbFriendsOnEQ.classesWithFriends !== null ? _.chain(data.fbFriendsOnEQ.classesWithFriends).map(function (item) { return item.friends[0].userId; }).uniq().value().length : 'zero') === 0 ? 'zero' : _.chain(data.fbFriendsOnEQ.classesWithFriends).map(function (item) { return item.friends[0].userId; }).uniq().value().length.toString()) : 'zero'
            };
