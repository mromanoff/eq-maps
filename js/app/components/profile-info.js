(function (App) {
    ...
    /*global EQ, Backbone, _, debug, APIEndpoint, moment*/
    ...
    App.Components['profile-info'] = function ($el) {
        ...
        var BikeStep = Backbone.View.extend({
            ...
            showStep: function () {
                var selector,
                    el = this.$el.find('.settings-container .' + this.stepId);
        ...
        var BikeSettings = Backbone.View.extend({
            ...
            startBikeSettings: function () {
                ...
                this.bikeSteps.push(
                    new BikeStep({el: bikeStepEl, value: '', id: 'seat-height'})
                );
                this.bikeSteps.push(
                    new BikeStep({el: bikeStepEl, value: '', id: 'seat'})
                );
                this.bikeSteps.push(
                    new BikeStep({el: bikeStepEl, value: '', id: 'handlebar-height'})
                );
                this.bikeSteps.push(
                    new BikeStep({el: bikeStepEl, value: '', id: 'handlebar'})
                );
            ...
            changeBackStepName: function () {
                var backBtn = this.$el.find('.controls #back-btn'),
                    backTxt = backBtn.data('alt');
            ...
            render: function () {
                ...
                EQ.Helpers.getService('/v2.6/me/preferences/bike').done(function (data) {
                    ...
                    if (self.bikeSettings.isDefault) {
                        ...
                        self.$el.find('.containers-row .seat-height .input span').text(
                            self.bikeSettings.preferences.seatHeight
                        );
                        self.$el.find('.containers-row .seat .input span').text(
                            textValues[self.bikeSettings.preferences.seatPosition]
                        );
                        self.$el.find('.containers-row .handlebar-height .input span').text(
                            self.bikeSettings.preferences.handlebarHeight
                        );
                        self.$el.find('.containers-row .handlebar .input span').text(
                            textValues[self.bikeSettings.preferences.handlebarPosition]
                        );
                    }
                }).fail(function () {
                    self.loaderAndError.showError();
