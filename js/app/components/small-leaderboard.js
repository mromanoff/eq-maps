(function (App) {
    /* global EQ, APIEndpoint3, APIEndpoint23, user, userProfileJson */
        ...
        */
        ...
        ActivityCyclingDetailRankView = Backbone.View.extend({
            ...
            render: function (stats, gender) {
                ...
                // Sort by energy.
                //rankingCollection.comparator = function (item) {
                //   return -item.get('energy');
                //};
                //rankingCollection.sort();
                ...
                //If the user exists and falls outside the rank, add him at the top
                console.log(jsonModel.me && (rank > jsonModel.leaderboard.length), rank, jsonModel.leaderboard.length);

                if (jsonModel.me && (rank > jsonModel.leaderboard.length)) {
                    var me = new Backbone.Model(jsonModel.me),
                        meView = new RankingSingleView({ model: me });
...
                        }
        ...
        */
        ...
        if (leaderboardsData.userCount === 0) {
            ...
            if (userGender) { // First time loading, if gender is there render with that.
    ...
    App.Components['small-leaderboard'] = function ($el, options) {
        ...
        var xhrStats = EQ.Helpers.getService(endpoint.CLASS_STATS(options.classId)),
            xhrRankings,
            leaderboard,
            currentRankingXhr;

        if (userGender) {
            xhrRankings = EQ.Helpers.getService(endpoint.CLASS_RANKINGS(options.classId) + '/' + userGender, { numResults : 4 });
        } else {
            xhrRankings = EQ.Helpers.getService(endpoint.CLASS_RANKINGS(options.classId), { numResults : 4 });
        }
        ...
        Backbone.Events.on('small-leaderboard:refresh', function (gender) {
            ...
            currentRankingXhr = EQ.Helpers.getService(endpoint.CLASS_RANKINGS(options.classId) + g, { numResults : 4 });
