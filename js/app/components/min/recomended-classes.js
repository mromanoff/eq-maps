(function(global, App) {
    ...
    var RecommendedClassesCarousel = Backbone.View.extend({
        ...
        render: function() {
            ...
            this.collection.each(function(classDetail) {
                var classSingleView = new ClassSingleView({
                    model: classDetail
                });
    ...
    var ClassSingleView = Backbone.View.extend({
        ...
        addClass: function(e) {
            e.preventDefault();
            var self = this, ENDPOINT, loaderAndError;
            ...
            if (!self.requestExecuting) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var timeOffset = moment(self.model.attributes.startDate).diff(currentDateTime, "hours"), timeOffsetMinutes = moment(self.model.attributes.startDate).diff(currentDateTime, "minutes");
                        if (timeOffset || timeOffsetMinutes) {
                            if (timeOffset > 0) {
                                timeOffset = Math.floor(timeOffset);
                            } else {
    ...
    App.Components["recomended-classes"] = function($el, options) {
        ...
        var recommendedClassesCollection = new RecommendedClassesCollection(), loaderAndError;
        _.extend(options, {
            showFilters: true
        });
