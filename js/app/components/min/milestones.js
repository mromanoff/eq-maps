(function(App) {
    ...
    App.Components.milestones = function($el) {
        ...
        var MilestonesView = Backbone.View.extend({
            ...
            render: function() {
                var totalMiles = this.options.totalMiles, isTrackSet = false;
                this.setHeaderBarColor();
                this.collection.each(function(milestone) {
                    var hasTrack = false, milestonesSingleView;
                    ...
                    if (milestone === this.collection.first() && milestone.get("value") === .01) {
            ...
            positionMilesTrack: function($el) {
                ...
                for (var i = 0; i < collectionModels.length; i++) {
                    if (collectionModels[i].get("isAchieved") === true && collectionModels[i + 1] !== undefined && collectionModels[i + 1].get("isAchieved") === false) {
                        var milestoneStart = this.options.totalMiles - collectionModels[i].get("value"), milestoneFinish = collectionModels[i + 1].get("value") - collectionModels[i].get("value"), trackAxis = $(window).width() < 768 ? "top" : "left";
        ...
        var MilestonesSingleView = Backbone.View.extend({
            ...
            viewMilestone: function() {
                var milestoneOverlaySettings, name = this.model.get("name");
                ...
                if (this.model.get("isAchieved") !== true) {
                    ...
                    var achievedDate = moment(this.model.get("achievedDetail").dateAchieved), milesValue = this.model.get("value") === .01 ? 0 : this.model.get("value");
                    ...
                    Backbone.Events.trigger("milestone-overlay:open", {
                        settings: milestoneOverlaySettings
                    });
                }
            }
        });
        ...
        var mileStonesHeroView = new MileStonesHeroView({
            el: $(".cycling-milestones-hero")
        });
        ...
        EQ.Helpers.getService("/v3/me/activity/cycling/milestones").done(function(data) {
            ...
            var milestonesCollection = new MilestoneCollection(data.milestones), milestoneListView = new MilestonesView({
            ...
            Backbone.Events.on("milestone-overlay:open", function(settings) {
                var milestoneOverlayView = new MilestoneOverlayView({
                    settings: settings,
                    totalMiles: data.totalMiles,
                    collection: milestonesCollection
                });
