(function(App) {
    "use strict";
    var overlayTemplate = "" + '<div class="automatic-calories-container">' + '<a href="#" class="icon-close close-overlay"></a>' + '<div class="personal-info-container">' + "<p>Please enter your current weight:</p>" + '<div class="module-container personal-info">' + '<div class="loader"></div>' + "</div>" + "</div>" + '<button class="button box small black log-weight">Save changes</button>' + '<button class="button box small white cancel-weight">Cancel</button>' + "</div>";
    var profileInfoTemplate = "" + '<div class="half-container weight right single" data-value="<%= weight.weight %>">' + '<div class="info">' + '<h3><%= weight.weight %><span class="suffix"><%= weight.weightUnit %></span></h3>' + '<div class="empty hidden"><span></span></div>' + "<span>Weight</span>" + "</div>" + '<div class="edit">' + '<div class="input-container">' + '<input type="text" name="weight" placeholder="<%= weight.weight %>" maxlength="4">' + '<label class="suffix"><%= weight.weightUnit %></label>' + "<span>Weight</span>" + "</div>" + "</div>";
    App.Components["log-weight-overlay"] = function($el) {
        ...
        var OverlayView = Backbone.View.extend({
            ...
            logWeight: function(e) {
                e.preventDefault();
                var self = this, loaderAndError = EQ.Helpers.loaderAndErrorHandler(self.$el.find(".log-weight"), {
