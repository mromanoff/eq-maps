(function(global, App) {
    ...
    var ReferAFriendView = Backbone.View.extend({
        ...
        render: function() {
            var referByEmail, referByLink;
            App.loadComponent("share", this.$el.find(".icons"), {
                type: "referrals",
                fbMode: "share-dialog",
    ...
    var ReferByEmail = Backbone.View.extend({
        ...
        send: function(e) {
            e.preventDefault();
            var referalForm = $("form", this.$el), self = this;
    ...
    var ReferByLink = Backbone.View.extend({
        ...
        render: function() {
            var url = this.model.get("url"), zClient;
    ...
    var FriendsView = Backbone.View.extend({
        render: function() {
            var count = 0, friends = [], maxItems = 5;
            this.$el.html("");
            console.log("collection", this.collection);
            this.collection.each(function(friend) {
                if (count < maxItems) {
                    var friendsSingleView;
                    friendsSingleView = new FriendsSingleView({
                        model: friend
                    });
            ...
            while (count < maxItems) {
                var emptyModel, emptySingleView;
                emptyModel = new Friend({
                    empty: true,
                    counter: count + 1
                });
                emptySingleView = new FriendsSingleView({
                    model: emptyModel
                });
    ...
    var FriendsSingleView = Backbone.View.extend({
        ...
        render: function() {
            var imgUrl, image = this.model.get("image");
    ...
    var ReferralsView = Backbone.View.extend({
        render: function() {
            var referrals = [], noReferralsMsg = $(".no-referred-msg", this.$el).removeClass("hidden");
            this.$el.html("");
            this.collection.each(function(referral) {
                var referalsSingleView = new ReferalsSingleView({
                    model: referral
                });
    ...
    var ReferalsSingleView = Backbone.View.extend({
        tagName: "li",
        render: function() {
            var fullName = this.model.get("firstName") + " " + this.model.get("lastName"), status = this.model.get("statusCode"), nameEl = $(document.createElement("strong"));
    ...
    Referrals.init = function($el, update) {
        var self = this, link, friendsCollection, referredCollection, referAFriendView, friendsView, referralsView, loaders;
        ...
        $.ajax(APIEndpoint + "/me/referrals", {
            ...
            if (!update) {
                link = new Link({
                    url: data.referredUrl
                });
                referAFriendView = new ReferAFriendView({
                    model: link
                });
