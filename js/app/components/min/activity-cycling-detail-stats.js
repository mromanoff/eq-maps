(function(global, App) {
    ...
    var endpoint = {
        ...
        GRAPH_STATS: function(classInstanceID) {
            return "/v3/classes/" + classInstanceID + "/me/data";
        },
    ...
    var ActivityCyclingDetailStatsView = Backbone.View.extend({
        ...
        getGraphOptions: function(datasets) {
            var steps = 30, scaleSteps = EQ.Helpers.getDeviceSize() === "small" ? 4 : 8, scaleMaxData = _.max(_.flatten(_.pluck(datasets, "data"))), scaleNumOfZeroes = Math.floor(Math.log(scaleMaxData) / Math.LN10), scaleMaxValue = Math.ceil(scaleMaxData / (1 * Math.pow(10, scaleNumOfZeroes))) * Math.pow(10, scaleNumOfZeroes), stepsWidth = Math.round(scaleMaxValue / scaleSteps);
            return {
        ...
        changeGraphMode: function(e) {
            ...
            if (!$currentOption.hasClass("selected")) {
                var ndata;
                $currentOption.addClass("selected").parent().siblings().find("a").removeClass("selected");
        ...
        updateMetrics: function(metricType) {
            var $statsBox = this.$el.find(".graphic-stats"), metricUnit = metricType === "energy" ? "AVG WATTS" : "MI", bestRideValue = metricType === "energy" ? parseInt(this.statsGraph.datasets[metricType][0].totalValue, 10) : EQ.Helpers.numbers.roundToTwoDecimals(this.statsGraph.datasets[metricType][0].totalValue), currentRideValue = metricType === "energy" ? parseInt(this.statsGraph.datasets[metricType][1].totalValue, 10) : EQ.Helpers.numbers.roundToTwoDecimals(this.statsGraph.datasets[metricType][1].totalValue);
    ...
    var utils = {
        getSpecificMetric: function(serviceData, metric) {
            var points = _.where(serviceData.metrics, {
                name: metric
            })[0].dataPoints, pointsToSimulate = [], lastPointValue, differenceBetweenPoints, step;
        ...
        parseGraphData: function(currentClassData, bestWattData, bestDistanceData) {
            var time, minutes = 0, seconds = 0, finalMetrics = {
                data: {
                    thisRide: {
                        Energy: {
        ...
        checkIfHasMetrics: function(graphData) {
            if (utils.getSpecificMetric(graphData, "AverageWatts").length === 0 && utils.getSpecificMetric(graphData, "TotalDistance").length === 0) {
                return false;
            } else {
                return true;
            }
    ...
    App.Components["activity-cycling-detail-stats"] = function($el, options) {
        require([ "/assets/js/app/amd_components/chart.js" ], function() {
            ...
            EQ.Helpers.when(xhrActivityCycling, xhrActivityCyclingThisRide).done(function(generalDataResponse) {
                var bestClassMetricsData = generalDataResponse[0].bestClassMetrics || [], currentClassMetrics = generalDataResponse[1].metrics, bestDistanceClass, bestWattsClass, xhrBestWatt, xhrBestDistance, xhrCurrentClass;
                bestDistanceClass = _.where(bestClassMetricsData, {
                    name: "TotalDistance"
                })[0];
                debug("BEST DISTANCE CLASS ID", bestDistanceClass);
                bestWattsClass = _.where(bestClassMetricsData, {
                    name: "AverageWatts"
                })[0];
                ...
                EQ.Helpers.when(xhrBestWatt, xhrBestDistance, xhrCurrentClass).done(function(response) {
                    var detailStatsComponent, bestWattData = response[0], bestDistanceData = response[1], currentClassData = response[2];
                    if (utils.checkIfHasMetrics(currentClassData)) {
                        ...
                        currentClassData.energyValue = _.where(currentClassMetrics, {
                            name: "AverageWatts"
                        })[0].value;
                        currentClassData.distanceValue = _.where(currentClassMetrics, {
                            name: "TotalDistance"
                        })[0].value;
