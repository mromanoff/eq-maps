(function(global, App) {
    "use strict";
    var CyclingLeaderboard = {}, CyclingLeaderboardView, CyclingLeaderboardHeroView, RankingSingleView, classId, isFiltered = false, endpoint = {
        CLASS_RANKINGS: function(classInstanceID, gender) {
            return typeof gender !== "string" ? "/v3/classes/" + classInstanceID + "/leaderboard" : "/v3/classes/" + classInstanceID + "/leaderboard/" + gender;
        }
    };
    CyclingLeaderboard.init = function(id, gender) {
        var cyclingLeaderboardHero, leaderboardData = EQ.Helpers.getService(endpoint.CLASS_RANKINGS(id, gender)), self = this;
        classId = id;
    ...
    CyclingLeaderboard.displayData = function(response) {
        var cyclingLeadersComponent, cyclingLeadersModel = new Backbone.Model(response);
        cyclingLeadersComponent = new CyclingLeaderboardView({
            model: cyclingLeadersModel
        });
    };
    ...
    CyclingLeaderboardHeroView = Backbone.View.extend({
        ...
        filter: function(e) {
            var $this = $(e.target), leaderboardData, f;
            e.preventDefault();
    ...
    CyclingLeaderboardView = Backbone.View.extend({
        ...
        render: function() {
            var rankingCollection = new Backbone.Collection(this.model.get([ 0 ]).leaderboard), userData = this.model.get([ 0 ]).me, rankingSingleView, $rankingContainer, className, userRank, row = 1;
            ...
            rankingCollection.each(function(user) {
                ...
                rankingSingleView = new RankingSingleView({
                    model: user,
                    className: className
                });
