(function(global, App) {
    ...
    var CalendarContainerView = Backbone.View.extend({
        ...
        showCalendar: function(e) {
            ...
            this.$el.find(".calendar-detail").add(".calendar-box").removeClass("hidden");
        ...
        showWorkout: function(e) {
            e.preventDefault();
            this.$el.find(".calendar-detail").add(".calendar-box").addClass("hidden");
        ...
        render: function() {
            var upcomingClassView, calendarIsEmpty = this.collection.toJSON()[0].calendarIsEmpty, calendarDays = this.collection.toJSON()[0].eventsByDay, upcomingClass = this.collection.toJSON()[0].upcomingEvent;
            if (calendarIsEmpty) {
                this.$el.find(".no-classes-overlay").removeClass("hidden");
            }
            ...
            if (upcomingClass) {
                upcomingClassView = new UpcomingClassView({
                    model: new UpcomingClass(upcomingClass)
                });
        ...
        renderActivity: function(visits) {
            var now = moment().startOf("month"), visitsCopy = visits === 1 ? " check-in" : " check-ins";
    ...
    var CalendarSmall = {}, WorkoutsCategories = null;
    CalendarSmall.init = function($el) {
        var currentWeek = EQ.Helpers.dateTime.getCurrentWeek(), today = new Date(), fromDate = currentWeek.startDate, toDate = currentWeek.endDate, now = today.getFullYear() + "-" + (today.getMonth() + 1) + "-" + today.getDate(), url = APIEndpoint26 + "/me/calendar/preview?fromDate=" + fromDate + "&toDate=" + toDate + "&now=" + now, $expandCollapseButton = $el.find("button.expand-collapse"), hasCollapsed = localStorage.getItem("calendar_collapsed"), loaderAndError;
        ...
        $expandCollapseButton.on("click", function(e) {
            ...
            if ($buttonExpand.hasClass("icon-left-arrow")) {
                $buttonExpand.addClass("icon-right-arrow").removeClass("icon-left-arrow");
                localStorage.setItem("calendar_collapsed", false);
            } else {
                $buttonExpand.addClass("icon-left-arrow").removeClass("icon-right-arrow");
        ...
        $.when($.ajax({
            ...
            dataType: "json"
        }), EQ.Helpers.getService("/v2.6/workouts/categories")).done(function(calendardata, categoriesdata) {
            debug("[SMALL CALENDAR SERVICE OK]", calendardata[0]);
            WorkoutsCategories = categoriesdata[0];
            var calendarContainerView, parsedData = CalendarSmall.parseData(calendardata[0], fromDate);
    ...
    CalendarSmall.parseData = function(response, fromDate) {
        var parsedData = {}, calendarIsEmpty = false, fromDateMoment = moment(fromDate), eventsByDay = {}, newDay, dayCode;
        for (var i = 0; i < 7; i++) {
        ...
        if (response.eventsByDay.length) {
            _.each(response.eventsByDay, function(day) {
                var dayCode = day.dayOfWeek.substr(0, 2), dateMoment = eventsByDay[dayCode].date;
                if (moment(dateMoment.format("YYYY-MM-DD")).isSame(moment().format("YYYY-MM-DD"))) {
                ...
                _.each(day.events, function(event) {
                    ...
                    event.primaryCategory = _.findWhere(WorkoutsCategories, {
                        categoryId: "" + event.workoutCategoryId
                    });
