(function (App) {
    ...
    var overlayTemplate = '' +
        '<div class="automatic-calories-container">' +
            ...
            '<h1>Automatic Class Calories</h1>' +
            '<p>Equinox now provides estimated calories burned in classes, based on member’s age, gender, height and weight.</p>' +
            '<div class="personal-info-container">' +
                '<p>To activate, please update your profile:</p>' +
                '<div class="module-container personal-info">' +
                    '<div class="loader"></div>' +
                '</div>' +
            '</div>' +
            '<button class="button box small black activate-acce">Activate Now</button>' +
            '<button class="button box small white deactivate-acce">Not right now</button>' +
        '</div>';

    var dateDropdownTemplate = '<option value="<%= date %>"<%= active ? \' selected\' : \'\' %>><%= date %></option>',
        profileInfoTemplate = '' +
            '<div class="half-container age" data-value="<%= birthdate || \'\' %>">' +
                '<div class="info">' +
                    '<h3><%= age %></h3>' +
                ...
                '</div>' +
            '</div>' +
            '<div class="half-container height right" data-value="<%= height.height %>">' +
                '<div class="info imperial">' +
                    '<h3><%= height.feet || \'\' %><span class="suffix">ft</span> <%= height.inch || \'\' %><span class="suffix">in</span></h3>' +
                    '<div class="empty hidden"><span></span></div>' +
                    '<span>Height</span>' +
                '</div>' +
                '<div class="info metric">' +
                    '<h3><%= height.height %><span class="suffix"><%= height.heightUnit %></span></h3>' +
                    '<div class="empty hidden"><span></span></div>' +
                    '<span>Height</span>' +
                '</div>' +
                '<div class="edit">' +
                    '<div class="input-container imperial">' +
                        '<input type="text" class="feet" name="heightft" placeholder="<%= height.feet || \'\' %>" maxlength="2">' +
                        '<label class="suffix">ft</label>' +
                        '<input type="text" class="inches" name="heightin" placeholder="<%= height.inch || \'\' %>" maxlength="2">' +
                        '<label class="suffix">in</label>' +
                        '<span>Height</span>' +
                    '</div>' +
                    '<div class="input-container metric">' +
                        '<input type="text" name="height" placeholder="<%= height.height %>" maxlength="4">' +
                        '<label class="suffix"><%= height.heightUnit %></label>' +
                        '<span>Height</span>' +
                    '</div>' +
                '</div>' +
            '</div>' +
            '<div class="half-container weight right" data-value="<%= weight.weight %>">' +
                '<div class="info">' +
                ...
                '</div>' +
            '</div>' +
            '<div class="half-container gender right" data-value="<%= gender || \'\' %>">' +
                '<div class="info">' +
                    '<h3><%= gender ? gender.substr(0,1) : "" %></h3>' +
                    '<div class="empty hidden"><span></span></div>' +
                    '<span>Gender</span>' +
                '</div>' +
                '<div class="edit">' +
                    '<div class="input-container">' +
                        '<label class="gender-select<%= gender && gender.substr(0,1) === \'M\' ? \' selected\' : \'\' %>">M<input type="checkbox" name="gender" value="Male"></label> / <label class="gender-select<%= gender && gender.substr(0,1) === \'F\' ? \' selected\' : \'\' %>">F<input type="checkbox" name="gender" value="Female"></label>' +
                        '<span>Gender</span>' +
                    '</div>' +
                '</div>' +
            '</div>';
    ...
    App.Components['automatic-calories-overlay'] = function ($el) {
        ...
        var ProfileInfoView = Backbone.View.extend({
            ...
            saveInfo: function (cb, cberror) {
                ...
                birthdate = $('.age .day-select').val() + ' ' +
                            $('.age .month-select').val() + ' ' +
                            $('.age .year-select').val();
        ...
        var OverlayView = Backbone.View.extend({
            ...
            activateAcce: function (e) {
                ...
                var self = this,
                    loaderAndError = EQ.Helpers.loaderAndErrorHandler(self.$el.find('.activate-acce'), {
