    ...
    App.Components['classes-filter'] = function ($el, options) {
        ...
        var LocationDropdownView = Backbone.View.extend({
            ...
            show: function () {
                ...
                // If all given facilities are selected, then mark it as selected
                that.$el.find('ul[data-facilities]').each(function (i, el) {
                    var $children = $(el).find('li a'),
                        $selected = $children.filter('.selected'),
                        region = $(el).data('facilities');
                ...
                that.$el.find('ul[data-region]').each(function (i, el) {
                    var $children = $(el).find('.top.region-title .deepselect'),
                        $selected = $children.filter('.selected'),
            ...
            selectAll: function (e) {
                ...
                var $facilities = $('ul[data-facilities="' + $li.data('region') + '"] li a'),
                    $region = $('ul[data-region="' + $li.data('region') + '"]'),
                    $items = [];
                ...
                if ($(e.currentTarget).hasClass('selected') === false) {
                    ...
                    _.each($items, function (e) {
                        ...
                        if (_.some(that.selectedItems, { id: item.id })) {
                            that.selectedItems = _.reject(that.selectedItems, { id: item.id });
                            Backbone.Events.trigger('classes-list:remove-filters:' + item.id);
                            $item.removeClass('selected');
                        }
                    });
        ...
        CategoriesDropdownView = Backbone.View.extend({
            ...
            addFilters: function () {
                if (this.interacted) {
                    ...
                    var classes = _.where(this.selectedItems, { 'isCatogory': false });
                    if (classes) {
                        // Add class items to filters
                        classFilters = {
                            classes: classes
                        };
                    ...
                    var categories = _.where(this.selectedItems, { 'isCatogory': true });
                    if (categories) {
                        // Add category items to filters
                        categoryFilters = {
            ...
            selectAll: function (e) {
                ...
                var $its = this.$el.find('ul[data-items="' + $li.data('items') + '"] li a'),
                    $items = [];
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            var $cats = $('<ul data-items="categories"></ul>'),
                                sorted = _.sortBy(data, 'categoryName');
