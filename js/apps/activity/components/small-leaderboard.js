(function (App) {
    ...
    App.Components['small-leaderboard'] = function ($el, options) {
        ...
        var xhrRankings = EQ.Helpers.getService(endpoint.CLASS_RANKINGS(options.classId), { numResults : 5 });

        EQ.Helpers.when(xhrStats, xhrRankings).done( function ( response ) {
            var leaderboard = new ActivityCyclingDetailRank($el, response[0], response[1], user);
